...
export default function Verifica() {
  ...
  const [estado, setEstado] = useState({ cargando: true, ok: false, msg: "" });

  useEffect(() => {
    const token = params.get("token");
    if (!token) {
      setEstado({ cargando: false, ok: false, msg: "Falta el token en la URL." });
      return;
    }
    (async () => {
      try {
        ...
        if (!res.ok || !data?.ok) throw new Error(data?.mensaje || "No se pudo verificar el email.");
        setEstado({ cargando: false, ok: true, msg: "¡Email verificado correctamente!" });
        setTimeout(() => navigate("/login", { replace: true }), 1200);
      } catch (e) {
        setEstado({ cargando: false, ok: false, msg: e?.message || "Error al verificar." });
      }
    })();
  }, [params, navigate]);
          ...
          <div className="card shadow-sm">
            <div className="card-body">
              ...
              <div className="d-flex gap-2">
                ...
                <Link to="/olvide" className="btn btn-outline-secondary">¿Reenviar enlace?</Link>
              </div>
            </div>
          </div>
